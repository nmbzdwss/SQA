...
from .transformer import TransformerDecoderLayer, TransformerDecoder, MaskTransformerDecoder
from src.util import box_ops
import numpy as np
import matplotlib.pyplot as plt
from src.util.misc import accuracy, is_dist_avail_and_initialized, get_world_size
from src.models.sqa_utils import check_annotation
import time
import clip
from datasets.hico_text_label import hico_text_label, obj_text_label, hico_unseen_index, text_of_label, obj_list, \
    hico_sub_do_label
...
class SQA(nn.Module):
    def __init__(self, args, detr, detr_matcher):
        ...
        if self.args.no_interaction_decoder:
            ...
            if self.args.use_attn_mask:
                self.interaction_decoder = MaskTransformerDecoder(d_model=self.args.hidden_dim,
                                                              nhead=self.args.hoi_nheads,
                                                              norm=decoder_norm, return_intermediate=True, mode=args.mode)
            else:
    ...
    def forward(self, samples: NestedTensor, targets=None):
        ...
        if self.args.use_attn_mask:
            sub_boxes = {}
            obj_boxes = {}
            for n in range(outputs_class.shape[1]):
                probs = outputs_class[-1, n].softmax(-1)
                inst_scores, inst_labels = probs[:, :-1].max(-1)
                human_instance_ids = torch.logical_and(inst_scores > 0.3, inst_labels == 1).nonzero(as_tuple=False)
                obj_instance_ids = torch.logical_and(inst_scores > 0.3,
                                                     inst_labels != (0 or 1 or 12 or 26 or 29 or 30 or 45
                                                                     or 66 or 68 or 69 or 71 or 83 or 91)).nonzero(
                    as_tuple=False)
                sub_box = outputs_coord[-1, n][human_instance_ids].flatten(0, 1)
                obj_box = outputs_coord[-1, n][obj_instance_ids].flatten(0, 1)
                obj_classes = inst_scores[obj_instance_ids].flatten(0, 1)
                sub_classes = inst_scores[human_instance_ids].flatten(0, 1)
                i = j = 5
                if obj_classes.shape[0] < 5:
                    i = obj_classes.shape[0]
                if sub_classes.shape[0] < 5:
                    j = sub_classes.shape[0]
                obj_index = torch.topk(obj_classes, i, dim=0)[1]
                sub_index = torch.topk(sub_classes, j, dim=0)[1]
                obj_boxes[n] = obj_box[obj_index]
                sub_boxes[n] = sub_box[sub_index]
        ...
        for imgid in range(bs):
            ...
            if self.training:
                ...
                if self.args.use_hard_mining_for_relation_discovery:
                    ...
                    all_rel_reps =self.coarse_relation_feature_extractor(all_pairs, relation_feature_map,
                    ...
                    sampled_rel_reps = self.coarse_relation_feature_extractor(sampled_rel_pairs, relation_feature_map,
                                                                              outputs_coord[-1, imgid].detach(),
                                                                              inst_repr[imgid],
                                                                              obj_label_logits=outputs_class[-1, imgid],
                                                                              idx=imgid)
                ...
                rel_reps = self.coarse_relation_feature_extractor(rel_pairs, relation_feature_map,
                                                                  outputs_coord[-1, imgid].detach(), inst_repr[imgid],
                                                                  obj_label_logits=outputs_class[-1, imgid], idx=imgid)
            ...
            # >>>>>>>>>>>> relation classification <<<<<<<<<<<<<<<
            if self.args.use_attn_mask:
                head_boxes = outputs_coord[-1, imgid][sampled_rel_pairs[:, 0]]
                tail_boxes = outputs_coord[-1, imgid][sampled_rel_pairs[:, 1]]
                all_boxes = torch.cat([sub_boxes[imgid], obj_boxes[imgid], head_boxes, tail_boxes], dim=0)
                all_masks = self.generate_mask(all_boxes,
                                               feature_masks=memory_input_mask, idx=imgid)  # [1,?,H,W]
                begin = 0
                end = sub_boxes[imgid].shape[0]
                sub_boxes_masks = all_masks[:, begin:end]
                begin = begin + sub_boxes[imgid].shape[0]
                end = end + obj_boxes[imgid].shape[0]
                obj_boxes_masks = all_masks[:, begin:end]
                begin = begin + obj_boxes[imgid].shape[0]
                end = end + head_boxes.shape[0]
                head_boxes_mask = all_masks[:, begin:end]
                begin = begin + head_boxes.shape[0]
                end = end + tail_boxes.shape[0]
                tail_boxes_mask = all_masks[:, begin:]
                if sub_boxes_masks.shape[1] != 0:
                    sub_boxes_mask = sub_boxes_masks[:, 0]
                    for i in range(sub_boxes_masks.shape[1]):
                        sub_boxes_mask = torch.minimum(sub_boxes_mask, sub_boxes_masks[:, i])  # [bs,H,W]`
                else:
                    sub_boxes_mask = torch.ones((1, 1, memory_input_mask.shape[1], memory_input_mask.shape[2]),
                                                device=outputs_coord.device,
                                                dtype=bool)
                if obj_boxes_masks.shape[1] != 0:
                    obj_boxes_mask = obj_boxes_masks[:, 0]
                    for i in range(obj_boxes_masks.shape[1]):
                        obj_boxes_mask = torch.minimum(obj_boxes_mask, obj_boxes_masks[:, i])  # [bs,H,W]
                else:
                    obj_boxes_mask = torch.ones((1, 1, memory_input_mask.shape[1], memory_input_mask.shape[2]),
                                                device=outputs_coord.device,
                                                dtype=bool)
                inter_att_mask = torch.minimum(head_boxes_mask[-1], tail_boxes_mask[-1]).unsqueeze(0)

                sub_att_mask = torch.minimum(sub_boxes_mask.expand(*inter_att_mask.shape),
                                             inter_att_mask)
                obj_att_mask = torch.minimum(obj_boxes_mask.expand(*inter_att_mask.shape),
                                             inter_att_mask)

                inter_att_mask = inter_att_mask.flatten(2, 3).to(dtype=bool)
                sub_att_mask = sub_att_mask.flatten(2, 3).to(dtype=bool)
                obj_att_mask = obj_att_mask.flatten(2, 3).to(dtype=bool)
    ...
    def generate_mask(self, outputs_coord, feature_masks, idx):
        if outputs_coord.shape[0] == 0 or outputs_coord is None:
            return torch.ones((1, 1, feature_masks.shape[1], feature_masks.shape[2]), device=outputs_coord.device,
                              dtype=bool)
        xyxy_boxes = box_ops.box_cxcywh_to_xyxy(outputs_coord).clamp(0, 1)  # [?,4]
        new_xyxy_boxes = []
        h, w = (~feature_masks[idx]).nonzero(as_tuple=False).max(dim=0)[0] + 1
        new_xyxy_boxes.append(xyxy_boxes * torch.tensor([w, h, w, h]).to(device=outputs_coord.device,
                                                                         dtype=outputs_coord.dtype))
        new_xyxy_boxes = torch.cat(new_xyxy_boxes, dim=0).long()  # [?,4]
        mask = []
        for i in range(outputs_coord.shape[0]):  # num_queries
            w1, h1, w2, h2 = new_xyxy_boxes[i]
            w2 = torch.minimum(w2, torch.tensor(feature_masks.shape[2] - 1).to(device=outputs_coord.device,
                                                                               dtype=outputs_coord.dtype)).long()
            h2 = torch.minimum(h2, torch.tensor(feature_masks.shape[1] - 1).to(device=outputs_coord.device,
                                                                               dtype=outputs_coord.dtype)).long()
            cur_mask = torch.zeros((feature_masks.shape[1], feature_masks.shape[2]), device=outputs_coord.device,
                                   dtype=bool)
            if h1 > 0:
                cur_mask = cur_mask.index_fill(0, torch.arange(0, h1).to(device=outputs_coord.device), True)
            if h2 + 1 < cur_mask.shape[0]:
                cur_mask = cur_mask.index_fill(0, torch.arange(h2 + 1, cur_mask.shape[0]).to(
                    device=outputs_coord.device), True)
            if w1 > 0:
                cur_mask = cur_mask.index_fill(1, torch.arange(0, w1).to(device=outputs_coord.device), True)
            if w2 + 1 < cur_mask.shape[1]:
                cur_mask = cur_mask.index_fill(1, torch.arange(w2 + 1, cur_mask.shape[1]).to(
                    device=outputs_coord.device), True)
            mask.append(cur_mask)
        mask = torch.stack(mask, dim=0).view(1, outputs_coord.shape[0], mask[0].shape[-2],
                                             mask[0].shape[-1])  # [bs,?,H,W]
        return mask  # get

class SQACriterion(nn.Module):
...
class SQAPostProcess(nn.Module):
...
class RelationFeatureExtractor(nn.Module):
    def __init__(self, args, in_channels, resolution=5, out_dim=1024):
        ...
        fusion_query_dim = instr_hidden_dim
        # spatial feature
        if args.use_spatial_feature:
            spatial_in_dim, spatial_query_dim, spatial_out_dim = 8, 5, 64
            self.spatial_proj = make_fc(spatial_in_dim, spatial_out_dim)
            fusion_dim += spatial_out_dim
            fusion_query_dim += spatial_out_dim

        # clip semantic feature
        if self.args.use_clip_fusion_q:
            with torch.no_grad():
                ...
                self.obj_text_embedding = self.clip_model.encode_text(obj_text_inputs.to(device)).float()
                # image=preprocess(samples.tensor).unsqueeze(0).to(clip_model.device)

            del clip_model
            fusion_dim += (512 * 2)
            fusion_query_dim += 512

        # sub and obj relation location
        if self.args.use_ho_rel_location:
            self.location_embeddings = nn.Embedding(4, self.args.hidden_dim)
            fusion_dim += 256
        # fusion
    ...
    def forward(self, rel_pairs, features, boxes, inst_reprs, idx, obj_label_logits=None):
        ...
        if self.args.use_clip_fusion_q:
            sub_sem, obj_sem = self.compute_sem(obj_label_logits, rel_pairs, idx)
            tail_feats = torch.cat([tail_feats, obj_sem], dim=-1)
            head_feats = torch.cat([head_feats, sub_sem], dim=-1)
        ...
        if self.args.use_ho_rel_location:
            ho_re_location = self.generate_ho_re_location(rel_pairs, boxes)
            ho_re_location = self.location_embeddings(ho_re_location)
            relation_feats = torch.cat([relation_feats, ho_re_location], dim=-1)

        x = self.fusion_fc(relation_feats)
        return x

    def extract_spatial_layout_feats(self, xyxy_boxes):
        box_center = torch.stack([(xyxy_boxes[:, 0] + xyxy_boxes[:, 2]) / 2, (xyxy_boxes[:, 1] + xyxy_boxes[:, 3]) / 2],
                                 dim=1)
        ...
        spatial_feats = torch.cat([
            ...
            overlap, union, box_area[:, None, None].expand(*union.shape), box_area[None, :, None].expand(*union.shape)
            # overlap, union, subj, obj
        ], dim=-1)
        return spatial_feats

    def compute_sem(self, outputs_class, sampled_rel_pairs, imgid):
        sub_inx = torch.max(outputs_class[sampled_rel_pairs[:, 0]], dim=-1)[1]
        obj_inx = torch.max(outputs_class[sampled_rel_pairs[:, 1]], dim=-1)[1]
        sub_sem = []
        obj_sem = []
        for sub_ind in zip(sub_inx):
            sub_sem.append(self.obj_text_embedding[sub_ind])
        for obj_ind in zip(obj_inx):
            obj_sem.append(self.obj_text_embedding[obj_ind])
        sub_sem = torch.stack(sub_sem)
        obj_sem = torch.stack(obj_sem)
        # detr has 92 class number, now the clip class is only 81
        return sub_sem, obj_sem

    def generate_ho_re_location(self, rel_pairs, boxes):
        ho_re_location = torch.zeros(len(rel_pairs)).to(device=boxes.device).long()
        sub_boxes = boxes[rel_pairs[:, 0]]
        obj_boxes = boxes[rel_pairs[:, 1]]
        far = torch.logical_or(abs(sub_boxes[:, 0] - obj_boxes[:, 0]) > (sub_boxes[:, 2] + obj_boxes[:, 2]) / 2,
                               abs(sub_boxes[:, 1] - obj_boxes[:, 1]) > (
                                       sub_boxes[:, 3] + obj_boxes[:, 3]) / 2).nonzero(as_tuple=False)

        middle_x = torch.logical_and(
            torch.maximum(sub_boxes[:, 2], obj_boxes[:, 2]) / 2 < abs(sub_boxes[:, 0] - obj_boxes[:, 0]),
            abs(sub_boxes[:, 0] - obj_boxes[:, 0]) <= (
                    sub_boxes[:, 2] + obj_boxes[:, 2]) / 2)
        middle_y = torch.logical_and(
            torch.maximum(sub_boxes[:, 3], obj_boxes[:, 3]) / 2 < abs(sub_boxes[:, 1] - obj_boxes[:, 1]),
            abs(sub_boxes[:, 1] - obj_boxes[:, 1]) <= (
                    sub_boxes[:, 3] + obj_boxes[:, 3]) / 2)
        middle = torch.logical_or(middle_x, middle_y).nonzero(as_tuple=False)

        near_x = torch.logical_and(
            torch.minimum(sub_boxes[:, 2], obj_boxes[:, 2]) / 2 < abs(sub_boxes[:, 0] - obj_boxes[:, 0]),
            abs(sub_boxes[:, 0] - obj_boxes[:, 0]) <= torch.maximum(
                sub_boxes[:, 2], obj_boxes[:, 2]) / 2)
        near_y = torch.logical_and(
            torch.minimum(sub_boxes[:, 3], obj_boxes[:, 3]) / 2 < abs(sub_boxes[:, 1] - obj_boxes[:, 1]),
            abs(sub_boxes[:, 1] - obj_boxes[:, 1]) <= torch.maximum(
                sub_boxes[:, 3], obj_boxes[:, 3]) / 2)
        near = torch.logical_or(near_x, near_y).nonzero(as_tuple=False)
        close = torch.logical_or(abs(sub_boxes[:, 0] - obj_boxes[:, 0]) <= torch.minimum(
            sub_boxes[:, 2], obj_boxes[:, 2]) / 2, abs(sub_boxes[:, 1] - obj_boxes[:, 1]) <= torch.minimum(
            sub_boxes[:, 3], obj_boxes[:, 3]) / 2).nonzero(as_tuple=False)
        ho_re_location[close] = 0
        ho_re_location[near] = 1
        ho_re_location[middle] = 2
        ho_re_location[far] = 3

        return ho_re_location
