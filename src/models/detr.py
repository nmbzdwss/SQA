...
from .sqa import SQA, SQAPostProcess, SQACriterion
...
def build(args):
    ...
    if args.HOIDet and args.SQA_relation_head:
        model = SQA(args, detr=model, detr_matcher=matcher)
        criterion = SQACriterion(args, matcher)
        postprocessors = {'hoi': SQAPostProcess(args, model)}
    elif args.HOIDet:
