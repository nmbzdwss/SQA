...
from .sqa_utils import MultiheadAttention
...
class MaskTransformerDecoder(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
                 norm=None, return_intermediate=False, mode=0):
        super().__init__()
        self.norm = norm
        self.mode = mode
        self.return_intermediate = return_intermediate
        if mode == 0:
            self.n1, self.n2, self.n3, self.n4 = 3, 1, 1, 1
        elif mode == 1:
            self.n1, self.n2, self.n3, self.n4 = 2, 2, 1, 1
        elif mode == 2:
            self.n1, self.n2, self.n3, self.n4 = 1, 3, 1, 1
        elif mode == 3:
            self.n1, self.n2, self.n3, self.n4 = 0, 2, 2, 2
        elif mode == 4:
            self.n1, self.n2, self.n3, self.n4 = 1, 2, 1, 2
        self.nhead = nhead
        self.layer1 = TransformerDecoderLayer(d_model, nhead, dim_feedforward, dropout)
        self.layers = _get_clones(self.layer1, 6)
    ...
    def forward(self, tgt, memory,
                ...
                memory_role_embedding: Optional[Tensor] = None,
                query_structure_encoding: Optional[Tensor] = None,
                oh_mask: Optional[Tensor] = None,
                h_mask: Optional[Tensor] = None,
                o_mask: Optional[Tensor] = None,
                only_mask: Optional[bool] = None, ):
        output = tgt
        intermediate = []
        oh_mask = oh_mask.unsqueeze(1).expand(-1, self.nhead, -1, -1).flatten(0, 1)
        h_mask = h_mask.unsqueeze(1).expand(-1, self.nhead, -1, -1).flatten(0, 1)
        o_mask = o_mask.unsqueeze(1).expand(-1, self.nhead, -1, -1).flatten(0, 1)
        if only_mask:
            memory_mask = oh_mask
        n = 0
        for layer in self.layers:
            if n < self.n1:
                memory_mask = memory_mask
            elif n < self.n1 + self.n2:
                memory_mask = oh_mask
            elif n < self.n1 + self.n2 + self.n3:
                memory_mask = h_mask
            else:
                memory_mask = o_mask
            output = layer(output, memory, tgt_mask=tgt_mask,
